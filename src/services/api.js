const BASE_URL = window.location.hostname === "127.0.0.1" ? "http://localhost:3000" : "https://backend-m5wv.onrender.com";
...
export async function fetchAllTradePartners(country, type, year) {
  return fetchJson(
    `${TRADE_API}/all/partners?country=${country}&type=${type}&year=${year}`
  );
}

export async function fetchAllTradeSectors(country, type, year) {
  return fetchJson(
    `${TRADE_API}/all/sectors?country=${country}&type=${type}&year=${year}`
  );
}

export async function fetchBilateralPartners(from, to, type, year) {
  return fetchJson(
    `${TRADE_API}/bilateral/partners?from=${from}&to=${to}&type=${type}&year=${year}`
  );
}

export async function fetchBilateralSectors(from, to, type, year) {
  return fetchJson(
